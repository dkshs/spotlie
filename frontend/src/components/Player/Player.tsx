...
export function Player() {
  const { currentMusic, playMusic, musicState, pauseMusic, time } = useMusic();

  return (
    currentMusic && (
      <div className="fixed bottom-0 inset-x-0 bg-black/80 backdrop-blur-2xl z-[9999] group/time">
        <div className="h-0.5 bg-zinc-600">
          <div className="hidden absolute group-hover/time:flex justify-between inset-x-0 -top-6 max-w-[1600px] mx-auto">
        ...
        <div className="p-4 flex items-center justify-between max-w-[1600px] min-w-[320px] m-auto">
          <div className="flex group sm:w-[30%]">
            <div className="relative">
              <Image
                ...
                width={56}
              />
              <div className="hidden absolute justify-center items-center inset-0 group-hover:flex group-hover:bg-black/50">
                <button
                  type="button"
                  className="w-full h-full flex justify-center items-center focus:outline-none focus:ring-2 ring-blue-300 rounded-lg duration-300"
                >
                  <ArrowsOutSimple
                    size={32}
            ...
            <div className="flex flex-col gap-0.5 px-3 truncate">
              <Link
                ...
                className="truncate hover:text-blue-400 focus:outline outline-1 outline-blue-300 duration-300"
              >
                {currentMusic.title}
              </Link>
              <Link
